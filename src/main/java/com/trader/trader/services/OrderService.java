...
public class OrderService {
    ...
    public void checkOpenTrades() throws IOException, InterruptedException, URISyntaxException {
        Long activeTrades=orderLogRepository.countByOpenOrClosed();
        ...
        if (activeTrades!=null && activeTrades>0) {
            ...
            Document curr = orderLogRepository.findTopByOrderByIdDesc();
            if (Math.abs((bid-curr.getDouble("price"))/curr.getDouble("price")) > takeProfit) {
                System.out.println("SOLD ORDER: ");
                curr.put("open_or_closed", false);
                OrderLog temp = new OrderLog(curr);
                orderLogRepository.save(temp);
            }
        }
